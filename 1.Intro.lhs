...
Haskell is used in academia and in industry:
  ...
  * Git-annex is a tool to manage (big) data files under Git version control. It
    also provides a distributed file synchronization system
    (git-annex assistant).
...
*** Industry
  ...
  * Bluespec SystemVerilog (BSV) is a language for semiconductor design that is
    an extension of Haskell. Also, Bluespec, Inc.'s tools are implemented in
    Haskell. Note this is an example of a DSL embedded into Haskell.
  ...
  * GitHub implemented Semantic, an open-source library for analysis, diffing,
    and interpretation of untrusted source code, in Haskell.
...
*** Some more examples from Serokell
  ...
  * Hasura is an open-source GraphQL engine that gives you instant access to a
    GraphQL API for your data. In the GitHub repository of Hasura, Haskell is
    the most used language, followed by languages like TypeScript, JavaScript,
    Python, and Go. At the start of 2022, Hasura raised \$100 million.
  * Microsoft uses Haskell in Bond, a cross-platform framework for working with
    schematized data. The Haskell part of Bond is mainly gbc, a command-line
    code generation tool that can generate C++ and C# code using Bond. For a
    long time, Microsoft Research also significantly supported the Glasgow
    Haskell Compiler (GHC) by employing Simon Peyton Jones to work on it.
  * Tesla has been regularly hiring Haskell engineers and interns for some years
    already. There isn’t a lot of information out there about the exact
    specifics of their work. According to a comment in a job post made on
    Reddit, they use Haskell to generate C code that is then compiled into
    vehicle firmware.
  * Freckle is an online learning platform that adjusts to the skills of
    students. They use Haskell for all of their backend services and most of
    their tooling.
  * Galois is a company that specializes in the research and development of
    trustworthy software systems for fields where failure is unacceptable.
    One of their most interesting projects is Copilot, a stream-based DSL for
    writing and monitoring embedded C programs.
...
purposes, we can separate two popular use cases:
  ...
  2. The second is compilers and DSLs. On our list, the examples for this use
     case are projects like Bond, Semantic, Copilot.
...
This file is actually a /literate Haskell/ file with extension @.lhs@ where
every piece of code is indented with @>@ like this:

> a = 5

Normal code is written without indent and is split into /modules/ each in their
own file with extension @.hs@. To write a comment, a double dash is used:
...
foldl1 means fold left, and use the first value in the list as the initial value
for the accumulator.
...
The operator @.@ chains functions together. For example, here foo is a function
that takes a value. It adds 10 to it, multiplies the result of that by 4, and
...
You can use foldl or foldr to reduce a list: @foldl <fn> <initial value> <list>@
...
> data Color = Red | Blue | Green -- enum in Rust
...
These are all Maybes, but with different @a@s:
...
Note that x and y MUST be the same type, as they are both defined as being of
type parameter @a@. A typeclass does not state that different types in the
typeclass can be mixed together. So @isEqual Red 2@ is invalid even though 2 is
an Int which is an instance of Eq, and Red is a TrafficLight which is also an
instance of Eq.
...
Haskell can automatically make types part of Eq, Ord, Read, Show, Enum and
Bounded with the @deriving@ keyword at the end of the type declaration
...
The code in @sayHello@ will never be executed, however. The only action that
ever gets executed in the source file is the value of @main@. To run @sayHello@,
comment out the above definition of @main@, replace it with @main = sayHello@
and run @runhaskell <file name>@.

Or, if you are running this module in GHCi, it can run any IO action on demand:
...
> action' = putStrLn "This is a line" >> (getLine >>= (\input1 ->
...
Is is applicable for every /monad/:
