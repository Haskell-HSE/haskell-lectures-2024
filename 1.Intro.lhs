...
* Pugs is a compiler and interpreter for the Raku programming language (formerly
  Perl 6).
...
* Bluespec SystemVerilog (BSV) is a language for semiconductor design that is an
  ...
  Note this is an example of a domain specific language embedded into Haskell.
...
This file is actually a _literate Haskell_ file with extension `.lhs` where
every piece of code is indented with `>` like this:

> a = 5

Normal code is written without indent and is split into _modules_ each in their
...
that *either* the (==) function or the (/=) is defined and the other will be
'filled in' based on the typeclass definition.
